...
/* Andi */
FILE *sfile;
int sfile_current_track = 0;
struct timespec sfile_last_time;
bool sfile_first_reg_write = true;
/* Andi end */
...
uint8_t sfile_last_regs[16];
uint32_t last_tstate;
bool frame_start;

void sound_ay_write_file(uint8_t reg, uint8_t data, uint32_t tstates)
{
        if (!sfile_first_reg_write) {
                frame_start = false;
                if (sfile_last_regs[reg] == data) {
                        return;
                }
        } else {
                frame_start = true;
                /* beginning of frame, every interrupt */
        }

        sfile_first_reg_write = false;
        sfile_last_regs[reg] = data;

        uint8_t first_byte, second_byte;
        /* we only write bytes... if this is a timestamp, we clear bit 7 in the
         * first byte, which is then the high order byte of the time stamp,
           if it is a register, we set bit 7 in the first byte */
        if (frame_start) {
                printf("= Frame Start =\n");
                first_byte = 0;
                second_byte = 0;
                fwrite(&first_byte, 1, 1, sfile);
                fwrite(&second_byte, 1, 1, sfile);
                last_tstate = 0;
        }

        if (tstates - last_tstate < 100) {
                /* no time stamp */
                printf("----\n");
        } else {
                //last_tstate = tstates;
                uint32_t micros = (int)(tstates / 3.5469);
                first_byte = (micros & 0x7FFF) >> 8;
                second_byte = micros & 0xFF;
                fwrite(&first_byte, 1, 1, sfile);
                fwrite(&second_byte, 1, 1, sfile);
                if (micros > 32767) {
                        printf("error, longer than 32767 micros\n");
                }
                printf("%u\n", micros);
                last_tstate = tstates;
        }
        first_byte = (reg & 0x0F) | 0x80; 
        second_byte = data;
        fwrite(&first_byte, 1, 1, sfile);
        fwrite(&second_byte, 1, 1, sfile);
    ...
    case 0xfd:
      switch(h)
        ...
        write_dat:
          ...
          sound_ay_write_file(ay_current_reg,a,tstates);
          break;
        default:
    ...
    case 0xf6:
      switch(a&0xc0)
        ...
        case 0x80:	/* write */
          ...
          sound_ay_write_file(ay_current_reg,cpc_f4,tstates);
          break;
...
while((addr=GETWORD(ptr))!=0)
  ...
  /* ARE: for every track, we open a new sound file */
  sfile_current_track++;
  char sfile_name[32];
  if (sfile_current_track > 1) {
        fclose(sfile);

  }
  sprintf(sfile_name, "sregs_track_%d.ay3", sfile_current_track);
  sfile = fopen(sfile_name, "w");
  memset(sfile_last_regs, 0, sizeof(sfile_last_regs));
}
